#'@title OTU2Taxa
#'@description This function aggregates OTUs into their taxonomic characteristics (genus or level)and it  analyses the most significant selected genera into each table.
#' To each genera, the function returns the hypergeometric distribution function P(x>=X) to each count. The function also returns
#' filtered data  by  counts higher than one. In both cases, we implemented -log(p+0.05), then a higher value means more significant, i.e., it is an enrichment genus or family.
#'@param Selection list or data frame from VarSelection or dAB function
#'@param TaxonInfo data.frame with taxonomic table associated to Data input. For instance, if Data comes from 16_S level, TaxoInfo should be a data.frame with 16_S associated taxonomic information
#'@param tableName a character indicating the table name. For instance, if your data comes from 16_S, this parameter should be "16_S".
#' Note that, this argument must mutch with the names from the input list into LinkData function.
#'@param AnalysisLev It is a character indicating if data should be aggregate to genera or family level
#'@export OTU2Taxa
#'@return List. The first element of this list contains all the selected taxa with their associated value from the hyperg distribution -log(p+0.05); the second element of this
#'list have only taxas counting up to 1.
#'
#'
#'
...
#'\enumerate{
#'\item
#'\item Da Wei Huang, B. T. S., & Lempicki, R. A. (2009). Bioinformatics enrichment tools: paths toward the comprehensive functional analysis of large gene lists. Nucleic acids research, 37(1), 1.
