...
#' Statis is part of the PCA family and is based on singular value decomposition (SVD) and the
...
#'@param Data should be a list of dataframes or ExpressionSet data with the same length of the number of tables to be integrate.
#'  In each dataframe, the Observations (common elements on Statis) should be in rows and the variables should be in columns. Data also might be a MultiAssayExperiment object
#'  from MultiAssayExperiment package, a software for multi-omics experiments integration in Bioconductor.
#'@param Distance Vector indicating which distance (including scalar product) should be applied to each study. If is missing,
#'the scalar product is used. The vector lenght must be equal to the length of Data.
#'Distance options: ScalarProduct, euclidean,  manhattan, canberra, pearson, pearsonabs, spearman,
#'spearmanabs, mahalanobis, BrayCurtis distance (please, use option Bray). For binary data, the distance can be jaccard,
#'simple matching, sokal&Sneath, Roger&Tanimoto, Dice,  Hamman, Ochiai, Sokal&Sneath, Phi-Pearson,
#'Gower&Legendre. Note that, use pre-processing option as compositional and Euclidean is the same than use Aitchison distance for compositional data.
#'@param Center Logical. If TRUE, the data frame
#'is centered  by the mean. By default is FALSE. If you have tables with different characteristics (continous phenotypes, frecuencies,
#'compositional data), we strongly recomendate normalize datasets as a previous step through DataProcessing option.
#'@param Scale A logical value indicating whether the column vectors should be
#'standardized by the rows weight, by default is FALSE. Note that: all data into list will be scaled. If you don't need to normalize all dataset, you
#'could set this parameter as False and perform the normalization step externally using DataProcessing function. If you have tables with different characteristics (continous phenotypes, frecuencies,
#'compositional data), we strongly recomendate normalize datasets as a previous step through DataProcessing option.
#'@param CorrelVector Logical. If TRUE (default), the RV matrix is computed using vectorial correlation, else
#'the Hilbert-Smith distance is used.
#'@param nCluster this variable indicates if common elements on the dataset should be grouped (by default is zero, i.e. no-cluster).
#'@param cl_method categorical (pam or kmeans). pam is a robust version of classical kmeans algorithm.
...
if(all(apply((x <- sapply(Data, rownames)), 2, function(y)+ identical(y, x[,1])))==FALSE){
  stop("tables should be the same observations")
}

# Normalice Data by frec or by sd
# else use Normalize
...
# Z matrix is  the centering matrix used to calculate distances, see Abdi (2007)
# see Abdi 2007 (Multiple distances matrix)
...
#get data names
#put X row/col names using Data
...
if (length(Distance)==length(Data)) {
S<-lapply(1:length(Distance),function(i){ComputeDistance(X[[i]],Distance[i],Z)})
}
...
#####################################################
RQITot<-as.data.frame((apply(AP[,1:2]^2,1,sum)/apply(AP^2,1,sum))*100)
...
if(cl_method=="km"){
fit <- kmeans(ProjObs[,1:2], nCluster)
...
TableProjections <- lapply(seq(1:length(S)),function(i){TabProj(S[[i]],SvdComp,Studies[i],Observations)})
names(TableProjections)<-names(S)
#return projections


##Return of different slots
.Object<-new("DistStatis")
.Object@RV<-RV
...
.Object@RQO<-RQITot
.Object@TableProjections<-TableProjections
validObject(.Object)
return(.Object)
  }
